...
parser.add_argument('--resume', '-r', default='2024_01_25_11_00_52/VCM_drop_0.1_2_8_lr_0.1_seed_0v2t_map_best.t', type=str,help='resume from checkpoint')
...
def test1():
    ...
    with torch.no_grad():
        for batch_idx, (imgs, pids, camids) in enumerate(galleryloader):
            ...
            feat = net(input, input,input, input, test_mode[0], seq_len=seq_lenth)
    ...
    with torch.no_grad():
        for batch_idx, (imgs, pids, camids) in enumerate(queryloader):
            ...
            batch_num = input.size(0)
            input = Variable(input.cuda())
            feat = net(input, input, input, input, test_mode[1], seq_len=seq_lenth)
    ...
    start = time.time()
    # result = {'RGBg_f':gall_feat,'RGBg_pids':g_pids,'RGBg_camids':g_camids,
    # 'IRq_f':query_feat,'IRq_pids':q_pids,'IRq_camids':q_camids}
    # scipy.io.savemat('FANet_t2v_results.mat',result)
    
    # print('Save features in FANet_results.mat !')
...
def test2():
    ...
    # result = {'IRg_f':gall_feat,'IRg_pids':g_pids,'IRg_camids':g_camids,
    # 'RGBq_f':query_feat,'RGBq_pids':q_pids,'RGBq_camids':q_camids}
    # scipy.io.savemat('FANet_v2t_results.mat',result)
