...
class VideoDataset_train_sam(Dataset):
    """Video Person ReID Dataset.
    Note batch data has shape (batch, seq_len, channel, height, width).
    """
    sample_methods = ['evenly', 'random', 'all']
    ...
    def __getitem__(self, index):
        ...
        if self.sample == 'random':
            ...
            for index in number_ir:
                index = int(index)
                img_path = img_ir_paths[index]
                # sam_path = img_path.replace("Train","TrainSam")
                sam_path = img_path.replace("Train","Train_Sam")
                img = read_image(img_path)
                sam_img = read_image(sam_path)

                img = np.array(img)
                sam_img = np.array(sam_img)
                if self.transform is not None: 
                    img = self.transform(img)
                    sam_img = self.transform(sam_img)

                imgs_ir.append(img)
                sam_ir.append(sam_img)

            imgs_ir = torch.cat(imgs_ir, dim=0)
            sam_ir = torch.cat(sam_ir, dim=0)
            ...
            for index in number_rgb:
                ...
                imgs_rgb.append(img)
                sam_rgb.append(sam_img)

            imgs_rgb = torch.cat(imgs_rgb, dim=0)
            sam_rgb = torch.cat(sam_rgb, dim=0)

            return imgs_ir, pid_ir, camid_ir, imgs_rgb, pid_rgb, camid_rgb,sam_ir,sam_rgb
